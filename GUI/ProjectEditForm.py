    ...
    def __init__(self, conn, project=None):
        ...
        if project == None:
            ...
            if str(self.BusinessEntity.Button) == "None":
                ...
                self.Button.set(self.BusinessEntity.Button)

        self.ProjectOmschrijvingLabel = Label(
            master, text='Project Omschrijving: ')
        ...
        self.ProjectOmschrijving = Entry(
            master, textvariable=self.ProjectOmschrijvingVar)
    ...
    def Confirm(self):
        try:
            if self.BusinessEntity == None:
                project = Project.Project(None, self.ProjectOmschrijvingVar.get(
                ), self.ProjectIDVar.get(), self.Button.get(), True)
                if project.Button == '':
                    project.Button = None
