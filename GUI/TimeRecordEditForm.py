...
class TimeRecordEditForm:
    def __init__(self, conn, timeRecordView, Cache):
        ...
        self.ProjectsCombo = ttk.Combobox(
            master, width=50, textvariable=self.ProjectValue)
        ...
        self.RecordTypeCombo = ttk.Combobox(
            master, width=30, textvariable=self.RecordTypeValue)
        ...
        self.DescriptionTextBox = Entry(
            master, textvariable=self.DescriptionValue)
    ...
    def Confirm(self):
        ...
        timeString = Globals.ConvertToAmericanDate(
            self.TimeRecord.Date) + ' 00:00'
        ...
        if not self.ProjectsCombo.current() == -1:
            Tr.ProjectID = self.Cache.ActiveProjects[self.ProjectsCombo.current(
            )].ID
        if not self.RecordTypeCombo.current() == -1:
            Tr.RecordTypeID = self.Cache.RecordTypes[self.RecordTypeCombo.current(
            )].ID
